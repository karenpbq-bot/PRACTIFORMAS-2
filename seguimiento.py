...
def registrar_hitos_cascada(p_id, hito_final, fecha_str):
    ...
    try:
        ...
        hitos_con_data = [r['hito'] for r in existentes.data]
...
def mostrar(supervisor_id=None):
    st.markdown("""
        ...
        .scroll-area { max-height: 500px; overflow-y: auto; overflow-x: hidden; border: 1px solid #eee; padding: 15px; border-radius: 8px; }
        .metric-box { background-color: #f8f9fa; padding: 10px; border-radius: 8px; text-align: center; border: 1px solid #dee2e6; }
        </style>
    """, unsafe_allow_html=True)
    
    st.header("📈 Panel de Seguimiento Matricial V2")
    supabase = conectar()
    ...
    # Los botones de Exportar y Guardar se activarán tras seleccionar proyecto
    ...
    if not st.session_state.get('id_p_sel'):
        st.info("💡 Por favor, seleccione un proyecto en el Bloque 1."); return

    id_p = st.session_state.id_p_sel
    p_data = df_p_all[df_p_all['id'] == id_p].iloc[0]
    ...
    with st.expander("🛠️ BLOQUE 2: CONFIGURACIÓN AVANZADA Y HERRAMIENTAS", expanded=False):
        ...
        with t1:
            st.write("Asigne el peso (%) de cada etapa (Suma = 100%):")
            cols_w = st.columns(4)
        ...
        with t3:
            ...
            if f_av and st.button("🚀 Procesar Importación"):
                ...
                prods_db = pd.DataFrame(supabase.table("productos").select("id, ubicacion, tipo, ml").eq("proyecto_id", id_p).execute().data)
    ...
    segs = pd.DataFrame(supabase.table("seguimiento").select("*").in_("producto_id", prods_all['id'].tolist()).execute().data)
    ...
    def calc_avance(df_m, df_s):
        ...
        puntos = sum([pesos.get(h, 0) for h in df_s[df_s['producto_id'].isin(df_m['id'])]['hito']])
        return round(puntos / len(df_m), 2)

    pct_total = calc_avance(prods_all, segs)
    pct_parcial = calc_avance(df_f, segs)

    # --- 5. BOTONES DE ACCIÓN (CONTINUACIÓN) ---
    ...
    # Exportación (Obedeciendo al Excel de muestra)
    df_exp = df_f.copy()
    for h in HITOS_LIST:
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df_exp[['proyecto_id', 'ubicacion', 'tipo', 'ctd', 'ml'] + HITOS_LIST].to_excel(writer, index=False)
    ...
    def render_filas(df_r):
        ...
        for _, p in df_r.iterrows():
            ...
            for i, h in enumerate(HITOS_LIST):
                ...
                if cols[i+1].checkbox("", key=f"chk_{p['id']}_{h}", value=existe, disabled=bloqueo, label_visibility="collapsed"):
                    if not existe:
                        registrar_hitos_cascada(p['id'], h, fecha_reg.strftime("%d/%m/%Y"))
                        st.rerun()
                elif existe and not bloqueado: # Lógica de Desmarcado 8 a 1
    ...
    if agrupar_por != "Sin grupo":
        for n, g in df_f.groupby(agrupar_por.lower()):
            st.markdown(f"**📂 {n}**"); render_filas(g)
    else: render_filas(df_f)
    st.markdown('</div>', unsafe_allow_html=True)
